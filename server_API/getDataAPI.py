...
class MyServer(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if cli_query.query=='summaryStats':
            ...
            print(len(allFighters),' fighters got from DB')
        ...
        elif 'fighterName' in cli_query.query:
            print('search query recieved')
            ind = cli_query.query.find('=')+1
            fighterName = cli_query.query[ind:]

            nameInd = fighterName.find(',')+1
            fname = fighterName[0:nameInd-1]
            lname = fighterName[nameInd:]
            fighterID = self.get_fighterID(fname,lname)
            print(fighterID, 'fighterId in search query')

            if fighterID is not None:
                print(
                    f'fighters name is {fname, lname}, fighterID is {fighterID}')
               
                data = self.get_individualStatsData(fighterId=fighterID)
                ...
                self.send_header("Access-Control-Allow-Origin", "*")
                self.send_header(
                    'data', f'fighter with name {fname} {lname} is not in database')
                self.send_error(404, 'cannot find fighter', f'fighter with name {fname} {lname} is not in database')
                self.end_headers()

        elif 'fighterImage' in cli_query.query:
            print('search query recieved (IMG)')
            ind = cli_query.query.find('=')+1
            fighterName = cli_query.query[ind:]
            ...
            if fighterID is not None:
                imageData = self.get_individualStatsFig(fighterId=fighterID)
...
class dataHandler(MyServer):
    ...
    def get_fighterID(self,fname,lname):
        ...
        if val:
            fighterID = val[0]
            print(fighterID, fname, lname, 'in get_fighterID')
        return fighterID
